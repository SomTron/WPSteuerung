...
from utils import safe_timedelta, read_temperature, is_nighttime, ist_uebergangsmodus_aktiv
...
class State:
    def __init__(self, config):
        ...
        try:
            ...
            self.uebergangsmodus_start = datetime.strptime("00:00", "%H:%M").time()
            self.uebergangsmodus_ende = datetime.strptime("00:00", "%H:%M").time()
        ...
        self.last_config_hash = hashlib.md5(open("config.ini", "rb").read()).hexdigest()
...
def kompressor_status_func(state):
    """Returns the current compressor status from the state object."""
    return state.kompressor_ein

def current_runtime_func(state):
    """Returns the current runtime from the state object."""
    return state.current_runtime

def total_runtime_func(state):
    """Returns the total runtime for today from the state object."""
    return state.total_runtime_today
...
async def set_kompressor_status(state, ein: bool, force: bool = False, t_boiler_oben: Optional[float] = None):
    ...
    now = datetime.now(local_tz)
    max_attempts = 3
    attempt_delay = float(state.config["Heizungssteuerung"].get("GPIO_ATTEMPT_DELAY", 0.5))
    ...
    try:
        ...
        async with state.gpio_lock:
            ...
            # Prüfe zuerst den aktuellen physischen Zustand
            current_physical_state = GPIO.input(21)
            logging.debug(f"Aktueller physischer GPIO-Zustand: {current_physical_state}")

            if not force:
                # Wenn der gewünschte Zustand bereits physisch aktiv ist, aktualisiere nur den State
                if ein and current_physical_state == GPIO.HIGH:
                    logging.debug("Kompressor ist bereits physisch eingeschaltet - aktualisiere nur State")
                    if not state.kompressor_ein:
                        state.kompressor_ein = True
                        state.start_time = now
                        state.last_compressor_on_time = now
                        state.current_runtime = timedelta()
                        logging.info(f"State korrigiert: Kompressor als EIN gesetzt (war bereits physisch EIN)")
                    return True
                elif not ein and current_physical_state == GPIO.LOW:
                    logging.debug("Kompressor ist bereits physisch ausgeschaltet - aktualisiere nur State")
                    if state.kompressor_ein:
                        state.kompressor_ein = False
                        ...
                        state.total_runtime_today += state.last_runtime
                        logging.info(f"State korrigiert: Kompressor als AUS gesetzt (war bereits physisch AUS)")
                    return True

            # Sicherheitsprüfung (nur wenn Force=False)
            if not force and t_boiler_oben is not None and t_boiler_oben >= SICHERHEITS_TEMP:
            ...
            # Nur ändern, wenn sich der Zustand tatsächlich ändern soll
            if state.kompressor_ein == ein and current_physical_state == (GPIO.HIGH if ein else GPIO.LOW) and not force:
                logging.debug(f"Kompressorstatus unverändert: {'Ein' if ein else 'Aus'}")
                return True
            ...
            for attempt in range(max_attempts):
                ...
                if readback == target_gpio:
                    ...
                    logging.warning(
                        f"GPIO-Inkonsistenz im Versuch {attempt + 1}: Ziel={'HIGH' if ein else 'LOW'}, Gelesen={readback}")

            if not success:
                logging.critical(
                    f"Konnte GPIO 21 nicht auf {'HIGH' if ein else 'LOW'} setzen nach {max_attempts} Versuchen! Aktueller GPIO-Wert: {GPIO.input(21)}")
                if state.bot_token and state.chat_id and state.session:
                    await send_telegram_message(
                        ...
                        state.bot_token,
                        parse_mode=None  # Kein Markdown für Fehlermeldungen
                    )
                return False

            # Status erfolgreich aktualisieren
        ...
        # Fallback: Aktuellen physischen Zustand lesen und State anpassen
...
async def reload_config(session, state):
    try:
        ...
        logging.info(f"Mode loaded from state.json: bademodus_aktiv={state.bademodus_aktiv}, urlaubsmodus_aktiv={state.urlaubsmodus_aktiv}")
...
async def watchdog_gpio(state):
    while True:
        try:
            ...
            # Erlaube eine Toleranz von 5 Minuten für Mindestlaufzeit
            if actual_gpio != expected_gpio:
                now = datetime.now(pytz.timezone("Europe/Berlin"))
                if state.kompressor_ein and state.start_time:
                    elapsed_time = safe_timedelta(now, state.start_time, state.local_tz)
                    min_laufzeit = timedelta(seconds=int(state.config["Heizungssteuerung"].get("MIN_LAUFZEIT_S", 900)))

                    # Wenn Mindestlaufzeit noch nicht erreicht, ist dies normal
                    if elapsed_time.total_seconds() < min_laufzeit.total_seconds():
                        logging.debug(
                            f"GPIO-Überwachung: Mindestlaufzeit noch nicht erreicht ({elapsed_time.total_seconds():.1f}s von {min_laufzeit.total_seconds()}s)")
                        await asyncio.sleep(10)
                        continue

                # Echte Inkonsistenz melden
                logging.warning(
                    f"GPIO-Inkonsistenz erkannt: state.kompressor_ein={state.kompressor_ein}, GPIO={actual_gpio}")

                # Versuche State an physischen Zustand anzupassen
                if actual_gpio == GPIO.HIGH and not state.kompressor_ein:
                    logging.info("Passe State an: Kompressor ist physisch EIN, setze state.kompressor_ein=True")
                    state.kompressor_ein = True
                    state.start_time = now
                    state.last_compressor_on_time = now
                    state.current_runtime = timedelta()
                elif actual_gpio == GPIO.LOW and state.kompressor_ein:
                    logging.info("Passe State an: Kompressor ist physisch AUS, setze state.kompressor_ein=False")
                    state.kompressor_ein = False
                    set_last_compressor_off_time(state, now)
                    if state.last_compressor_on_time is not None:
                        state.last_runtime = safe_timedelta(now, state.last_compressor_on_time, state.local_tz)
                    else:
                        state.last_runtime = timedelta()
                    state.total_runtime_today += state.last_runtime
            ...
            if state.session and state.bot_token and state.chat_id:
                await send_telegram_message(
                    state.session, state.chat_id,
                    f"Fehler im GPIO-Watchdog: {str(e)}", state.bot_token,
                    parse_mode=None  # Kein Markdown für Fehlermeldungen
                )
...
async def check_sensors_and_safety(session, state, t_oben, t_unten, t_mittig, t_verd):
    ...
    if t_oben is not None and t_unten is not None and (
        ...
        state.ausschluss_grund = f"Uebertemperatur (>= {state.sicherheits_temp} Grad)"
        logging.error(f"Sicherheitsabschaltung: T-Oben={t_oben:.1f} Grad, T-Unten={t_unten:.1f} Grad")
        if state.kompressor_ein:
            ...
            if result:
                ...
                if state.last_overtemp_notification is None or safe_timedelta(now, state.last_overtemp_notification, local_tz) > timedelta(minutes=5):
                    await send_telegram_message(
                        session, state.chat_id,
        ...
        if state.last_overtemp_notification is None or safe_timedelta(now, state.last_overtemp_notification, local_tz) > timedelta(minutes=5):
            await send_telegram_message(
                session, state.chat_id,
                f"⚠️ Sicherheitsabschaltung: T-Oben={t_oben:.1f} Grad, T-Unten={t_unten:.1f} Grad >= {state.sicherheits_temp} Grad",
                state.bot_token,
                parse_mode=None
            )
            state.last_overtemp_notification = now
    ...
    if t_verd is not None and t_verd < state.verdampfertemperatur:
        ...
        if state.last_verdampfer_notification is None or safe_timedelta(now, state.last_verdampfer_notification, local_tz) > timedelta(minutes=5):
            await send_telegram_message(
                session, state.chat_id,
    ...
    #logging.debug(f"Safety checks passed: T-Oben={t_oben:.1f}, T-Unten={t_unten:.1f}, T-Verd={t_verd:.1f}")
    return True
...
async def determine_mode_and_setpoints(state, t_unten, t_mittig):
    ...
    from utils import is_nighttime  # Use is_nighttime from utils
    now = datetime.now(state.local_tz)
    is_night = is_nighttime(state.config)  # Updated to use is_nighttime
...
async def main_loop(config, state, session):
    """Hauptschleife des Programms mit globalem State-Objekt."""
    local_tz = pytz.timezone("Europe/Berlin")
    min_laufzeit = timedelta(seconds=int(state.config["Heizungssteuerung"].get("MIN_LAUFZEIT_S", 900)))
    ...
    try:
        ...
        # Map SENSOR_IDS keys to those expected by telegram_task
        telegram_sensor_ids = {
            "temp_oben": SENSOR_IDS["oben"],
            "temp_unten": SENSOR_IDS["unten"],
            "temp_mittig": SENSOR_IDS["mittig"],
            "temp_verd": SENSOR_IDS["verd"]
        }

        # Starte Telegram-Task
        telegram_task_handle = asyncio.create_task(telegram_task(
            read_temperature_func=read_temperature,
            sensor_ids=telegram_sensor_ids,
            config=config,
            state=state,
        ...
        while True:
            try:
                ...
                state.bademodus_aktiv = state_data.get("bademodus_aktiv", False)
                state.urlaubsmodus_aktiv = state_data.get("urlaubsmodus_aktiv", False)
                logging.debug(f"Mode updated: bademodus_aktiv={state.bademodus_aktiv}, urlaubsmodus_aktiv={state.urlaubsmodus_aktiv}")
                ...
                state.previous_modus = mode_info["modus"]  # Update state.previous_modus
