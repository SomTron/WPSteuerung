...
async def send_telegram_message(session, chat_id, message, bot_token, retries=3, retry_delay=5):
    """Sendet eine Nachricht über Telegram mit dynamischer Fehlerbehandlung und Wiederholungslogik."""
    if len(message) > 4096:
    ...
    for attempt in range(1, retries + 1):
            ...
            async with session.post(url, json=payload, timeout=20) as response:
                if response.status == 200:
                    ...
                    try:
                        ...
                        description = error_json.get("description", "Keine Beschreibung verfügbar")
                        retry_after = error_json.get("parameters", {}).get("retry_after")

                        if error_code == 429 and retry_after:
                            logging.warning(
                                f"Telegram Rate-Limit erreicht (Versuch {attempt}/{retries}). Warte {retry_after} Sekunden..."
                            )
                            await asyncio.sleep(retry_after)
                            continue  # Springe zur nächsten Wiederholung

                        logging.error(
                            f"Fehler beim Senden der Telegram-Nachricht (Status {response.status}, Versuch {attempt}/{retries}): {error_text}")

                    except Exception as json_e:
                        logging.error(f"Fehler beim Parsen der Telegram-Fehlerantwort: {json_e}")
                        logging.error(f"Rohantwort: {error_text}")
        ...
        # Normale Wartezeit, falls keine spezifische 'retry_after' Angabe gemacht wurde
        if attempt < retries:
            logging.info(f"Warte {retry_delay} Sekunden vor dem nächsten Versuch...")
            await asyncio.sleep(retry_delay)
        else:
            logging.error("Alle Versuche fehlgeschlagen.")
            return False
